...
/**
 *
 * Created by zhangye on 2017/12/22.
 */
class PermissionsManager private constructor() {
     ...
     */
    @Synchronized private fun initializePermissionsMap() {
        ...
        for (field in fields) {
            ...
            try {
                ...
                LogUtils.e("Could not access field", e)
            }
     ...
     */
    @Synchronized private fun getManifestPermissions(activity: Activity): Array<String> {
        var packageInfo: PackageInfo? = null
        val list = ArrayList<String>(1)
        try {
            LogUtils.d(activity.packageName)
            packageInfo = activity.packageManager.getPackageInfo(activity.packageName, PackageManager.GET_PERMISSIONS)
        } catch (e: PackageManager.NameNotFoundException) {
            LogUtils.e("A problem occurred when retrieving permissions", e)
        }
        ...
        if (packageInfo != null) {
            ...
            if (permissions != null) {
                for (perm in permissions) {
                    LogUtils.d("Manifest contained permission: " + perm)
                    list.add(perm)
                }
            }
        }
     ...
     */
    @Synchronized private fun addPendingAction(permissions: Array<String>,
     ...
     */
    @Synchronized private fun removePendingAction(action: PermissionsResultAction?) {
    ...
    companion object {
        ...
        val instance: PermissionsManager
            get() {
                ...
                return mInstance as PermissionsManager
            }
    }
